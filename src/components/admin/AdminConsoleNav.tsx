...
export default function AdminConsoleNav({
  ...
  return (
    <div
      className={cn(
        "mb-6 rounded-3xl p-4",
        isSurface
          ? "border border-border bg-background shadow-sm"
          : "border border-slate-800/80 bg-slate-950/70 shadow-2xl shadow-black/20"
      )}
    >
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className={cn(
            ...
            isSurface ? "text-muted-foreground" : "text-slate-500"
          )}>
            Panel admin
          </p>
          <p className={cn(
            "mt-1 text-sm",
            isSurface ? "text-muted-foreground" : "text-slate-300"
          )}>
            Operación, monitoreo y revisión de actividad real de TutorIA.
          </p>
        </div>
        <Badge
          variant="outline"
          ...
          if (item.disabled || !item.to) {
            return (
              <div
                key={item.key}
                className={cn(
                  "flex min-h-[72px] items-center justify-between rounded-2xl border px-4 py-3",
                  isSurface
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className={cn("text-xs", isSurface ? "text-muted-foreground" : "text-slate-500")}>Próximamente</div>
                  </div>
                </div>
                <Badge variant="outline" className={cn(isSurface ? "border-border text-muted-foreground" : "border-slate-700 text-slate-500")}>Soon</Badge>
              </div>
            );
          }
                ...
                <div>
                  ...
                  <div
                    className={cn(
                      "text-xs",
                      active
                        ? (isSurface ? "text-accent/80" : "text-cyan-200/80")
                        : (isSurface ? "text-muted-foreground" : "text-slate-500")
                    )}
                  >
                ...
                <Badge
                  className={cn(
                    isSurface
                      ...
                      : "border-cyan-400/30 bg-cyan-500/20 text-cyan-100 hover:bg-cyan-500/20"
                  )}
                >
                  Activo
                </Badge>
